...
Do Until rs.EOF	
		...
		employeeT(i) = UCase(rs("employeeType"))
		i=i+1
		rs.movenext
Loop
...
response.write "<th style='height:17px;padding:0;text-align:left;padding-left:5px;' scope='col'>Type:</th>"
...
if len(f)>0 or len(l)>0 or len(eid)>0 then
  for i=1 to ubound(first)
      ...
      if f = mid(first(i),1,len(f)) and l = mid(last(i),1,len(l)) and eid = mid(id(i),1,len(eid))then
		...
		response.write "<tr><td style='height:17px; padding:0;'>" & id(i) & "</td><td style='height:17px; padding:0;'>" & first(i) & "</td><td style='height:17px; padding:0;'>"  & last(i) & "</td><td style='height:17px; padding:0;'>" & employeeT(i) & "<td style='height:17px; padding:0;text-align:right;padding-right:10px;'>" & statusActive & "</td><td><button value='" & id(i) & "'type='button' style='font-size:8px;' onclick='editUser(this.value)'>Edit</button></td></tr>"
    end if
  next
end if
