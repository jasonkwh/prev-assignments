...
if len(o)>0 then
  for i=1 to ubound(offence)
      ...
      if o = mid(offence(i),1,len(o)) then
	  	if offenceaction(i) = 1 then
			oAction = "Fine"
		else if offenceaction(i) = 2 then
			oAction = "Court Order"
		else
			oAction = "Both"
		end if
		end if
		response.write "<tr><td style='height:17px; padding:0;'>" & offence(i) & "<td style='height:17px; padding:0;'>$"  & fineamount(i) & "<td style='height:17px; padding:0;'>" & oAction & "</td><td><button value='" & offence(i) & "'type='button' style='font-size:8px;' onclick='editOffence(this.value)'>Edit</button></td></tr>"
    end if
  next
end if
